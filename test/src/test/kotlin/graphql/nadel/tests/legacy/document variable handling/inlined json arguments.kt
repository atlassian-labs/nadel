...
public class `inlined json arguments` : NadelLegacyIntegrationTest(query = """
|query myQuery {
    ...
    """.trimMargin(), runtimeWiring = { wiring ->
      wiring.type("Query") { type ->
        type.dataFetcher("hello") { env ->
          if (env.getArgument<Any?>("arg") == mapOf("payload" to mapOf("name" to "Bobert", "age" to
              "23")) && env.getArgument<Any?>("arg1") == mapOf("interests" to listOf("photography",
              "basketball"))) {
            "world"}
          else {
            null}
