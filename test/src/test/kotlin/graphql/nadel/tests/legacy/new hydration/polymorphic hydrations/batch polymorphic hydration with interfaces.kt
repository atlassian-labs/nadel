...
public class `batch polymorphic hydration with interfaces` : NadelLegacyIntegrationTest(query = """
|query {
|  foo {
|    __typename
|    id
      ...
      wiring.type("Query") { type ->
        type.dataFetcher("petById") { env ->
          if (env.getArgument<Any?>("ids") == listOf("DOG-0", "FISH-0", "DOG-1", "FISH-1")) {
            listOf(Pets_Dog(id = "DOG-0", breed = "Akita"), Pets_Fish(id = "FISH-0", fins = 4),
                Pets_Dog(id = "DOG-1", breed = "Labrador"), Pets_Fish(id = "FISH-1", fins = 8))}
          else {
            null}
        }
      }
