...
public class `inlined all arguments with renamed field input is wrapped in a list` :
...
|  }
|}
|""".trimMargin(), variables = mapOf("varX" to listOf(mapOf("names" to listOf("Bobba", "Fett"),
    "payload" to mapOf("name" to "Bobert", "age" to "23")))), services =
    ...
    """.trimMargin(), runtimeWiring = { wiring ->
      wiring.type("Query") { type ->
        type.dataFetcher("helloUnderlying") { env ->
          if (env.getArgument<Any?>("arg") == listOf(mapOf("age" to 50, "inputWithJson" to
              listOf(mapOf("names" to listOf("Bobba", "Fett"), "payload" to mapOf("name" to
              "Bobert", "age" to "23")))))) {
  ...
  private data class MyService_UnderlyingInputArgType(
    ...
    public val inputWithJson: List<MyService_InputWithJsonUnderlying?>? = null,
  )
}
