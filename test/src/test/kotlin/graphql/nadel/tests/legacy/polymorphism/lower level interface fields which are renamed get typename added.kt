...
public class `lower level interface fields which are renamed get typename added` :
      ...
      wiring.type("Query") { type ->
        type.dataFetcher("pets") { env ->
          if (env.getArgument<Any?>("isLoyal") == true) {
            listOf(PetService_Dog(name = "Sparky", collar = PetService_DogCollar(color = "blue")),
                PetService_Cat(name = "Whiskers", collar = PetService_CatCollar(color = "red")))}
          else {
            null}
        }
      }
