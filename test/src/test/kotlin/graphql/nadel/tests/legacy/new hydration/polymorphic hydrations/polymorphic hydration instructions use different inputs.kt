...
public class `polymorphic hydration instructions use different inputs` :
      ...
      wiring.type("Query") { type ->
        type.dataFetcher("petsByIds") { env ->
          if (env.getArgument<Any?>("ids") == listOf("good-boye-1", "tall-boye-9")) {
            listOf(Pets_Pet(animalId = "good-boye-1", giraffeInput = null), Pets_Pet(animalId =
                "tall-boye-9", giraffeInput = Pets_PetGiraffeInput(nickname = "Tall Boye", birthday
                = 1_001_203_200, height = 570)))}
          else {
            null}
        }
      }
...
, Service(name="Dogs", overallSchema="""
    |type Query {
...
, Service(name="Cats", overallSchema="""
    |type Query {
    |  cats: CatQuery
    |}
    |
...
, Service(name="Zoo", overallSchema="""
    |type Query {
    ...
    |""".trimMargin(), runtimeWiring = { wiring ->
      wiring.type("Query") { type ->
        type.dataFetcher("giraffes") { env ->
          if (env.getArgument<Any?>("filters") == listOf(mapOf("nickname" to "Tall Boye", "birthday"
              to 1_001_203_200, "height" to 570))) {
            listOf(Zoo_Giraffe(birthday = 1_001_203_200, height = 570, name = "Rukiya", nickname =
                "Tall Boye"))}
          else {
            null}
        }
  ...
  private data class Pets_Pet(
    public val animalId: String? = null,
    public val giraffeInput: Pets_PetGiraffeInput? = null,
  )

  private data class Pets_PetGiraffeInput(
    public val nickname: String? = null,
    public val birthday: Int? = null,
  ...
  private data class Cats_CatQuery(
    public val catsByIds: List<Cats_Cat?>? = null,
  )
