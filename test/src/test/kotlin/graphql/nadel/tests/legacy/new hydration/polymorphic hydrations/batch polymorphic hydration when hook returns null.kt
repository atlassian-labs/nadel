...
public class `batch polymorphic hydration when hook returns null` : NadelLegacyIntegrationTest(query
    = """
|query {
|  foo {
|    __typename
    ...
    |""".trimMargin(), runtimeWiring = { wiring ->
      wiring.type("Query") { type ->
        type.dataFetcher("petById") { env ->
          if (env.getArgument<Any?>("ids") == listOf("PET-0")) {
            listOf(Pets_Pet(breed = "Akita", id = "PET-0"))}
          else {
            null}
        }
      }
      ...
      wiring.type("Query") { type ->
        type.dataFetcher("foo") { env ->
          listOf(Foo_Foo(dataId = "PET-0", id = "FOO-0"), Foo_Foo(dataId = "HUMAN-0", id = "FOO-1"),
              Foo_Foo(dataId = "NULL-1", id = "FOO-2"), Foo_Foo(dataId = "HUMAN-1", id = "FOO-3"))}
      }
    }
    )
)) {
