...
public class `namespaced result map has correct entries on partial failure` :
      ...
      wiring.type("Query") { type ->
        type.dataFetcher("issue") { env ->
          DataFetcherResult.newResult<Any>().data(null).errors(listOf(toGraphQLError(mapOf("message"
              to "Error", "locations" to emptyList<Any?>(), "extensions" to mapOf("classification"
              to "DataFetchingException"))))).build()}
      }
    }
    )
)) {
