...
public class `variable referenced input types rename works as expected` :
    NadelLegacyIntegrationTest(query = """
|query X(${'$'}var1: InputOverall!, ${'$'}var2: URL, ${'$'}var3: EnumOverall) {
|  renameInput(arg1: ${'$'}var1, arg2: ${'$'}var2, arg3: ${'$'}var3)
|}
|""".trimMargin(), variables = mapOf("var1" to mapOf("inputVal" to "x")), services =
    ...
    |""".trimMargin(), runtimeWiring = { wiring ->
      wiring.type("Query") { type ->
        type.dataFetcher("renameInput") { env ->
          if (env.getArgument<Any?>("arg1") == mapOf("inputVal" to "x")) {
            "done"}
          else {
            null}
        }
