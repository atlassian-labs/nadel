...
public class `batching no source inputs` : NadelLegacyIntegrationTest(query = """
|{
|  activity {
|    content {
|      __typename
    ...
    |""".trimMargin(), runtimeWiring = { wiring ->
      wiring.type("Query") { type ->
        type.dataFetcher("activity") { env ->
          listOf(Activity_Activity(contentIds = emptyList()), Activity_Activity(contentIds =
              emptyList()), Activity_Activity(contentIds = listOf("issue/7496", "comment/9001")),
    ...
    |""".trimMargin(), runtimeWiring = { wiring ->
      wiring.type("Query") { type ->
        type.dataFetcher("issuesByIds") { env ->
          if (env.getArgument<Any?>("ids") == listOf("issue/7496", "issue/1234")) {
            listOf(Issues_Issue(id = "issue/7496", title = "Seven Four Nine Six"), Issues_Issue(id =
                "issue/1234", title = "One Two Three Four"))}
          else {
            null}
        }
      }
