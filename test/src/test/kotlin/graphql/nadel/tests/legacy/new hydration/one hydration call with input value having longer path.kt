...
class `one hydration call with input value having longer path` : NadelLegacyIntegrationTest(
    query = """
        query {
          foo {
            barLongerInput {
            ...
            runtimeWiring = { wiring ->
                wiring.type("Query") { type ->
                    type.dataFetcher("foo") { env ->
                        Service1_Foo(fooDetails = Service1_FooDetails(externalBarId = "barId"))
                    }
                }
            },
        ),
