...
class `complex identified by with list source id` : NadelLegacyIntegrationTest(
    query = """
        query {
          issues {
            id
                ...
                wiring.type("Query") { type ->
                    type.dataFetcher("issues") { env ->
                        listOf(
                            Issues_Issue(
                                authorIds = listOf(
                                    Issues_UserRef(userId = "USER-1", site = "hello"),
                                    Issues_UserRef(userId = "USER-2", site = "hello"),
                                ),
                                id = "ISSUE-1",
                            ),
                            Issues_Issue(
                                authorIds = listOf(Issues_UserRef(userId = "USER-3", site = "hello")),
                                id = "ISSUE-2",
                            ),
                            Issues_Issue(
                                authorIds = listOf(
                                    ...
                                    Issues_UserRef(userId = "USER-4", site = "hello"),
                                    Issues_UserRef(
                                        userId = "USER-5",
                                        site = "hello",
                                    ),
