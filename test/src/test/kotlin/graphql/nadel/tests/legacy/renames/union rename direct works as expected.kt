...
public class `union rename direct works as expected` : NadelLegacyIntegrationTest(query = """
|query {
|  renameUnion {
|    ... on X {
|      x
...
|""".trimMargin(), variables = emptyMap(), services = listOf(Service(name="MyService",
    ...
    |""".trimMargin(), runtimeWiring = { wiring ->
      wiring.type("Query") { type ->
        type.dataFetcher("renameUnionUnderlying") { env ->
          MyService_XUnderlying(x = 1)}
      }
