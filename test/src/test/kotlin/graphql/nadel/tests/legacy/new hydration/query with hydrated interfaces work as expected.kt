...
public class `query with hydrated interfaces work as expected` : NadelLegacyIntegrationTest(query =
    """
      ...
      wiring.type("Query") { type ->
        type.dataFetcher("pets") { env ->
          if (env.getArgument<Any?>("isLoyal") == true) {
            listOf(PetService_Dog(name = "Sparky", ownerIds = listOf("dearly")), PetService_Cat(name
                = "Whiskers", ownerIds = listOf("cruella")))}
          else {
            null}
        }
      }
...
, Service(name="OwnerService", overallSchema="""
    ...
    |""".trimMargin(), runtimeWiring = { wiring ->
      wiring.type("Query") { type ->
        type.dataFetcher("ownerById") { env ->
          if (env.getArgument<Any?>("id") == "dearly") {
            OwnerService_CaringOwner(name = "Mr Dearly", givesPats = true)}
          else if (env.getArgument<Any?>("id") == "cruella") {
            OwnerService_CruelOwner(name = "Cruella De Vil", givesSmacks = true)}
          else {
            null}
        }
      }
