...
public class
    `query with pass through interfaces and unions that DONT have typename in them work as expected`
    ...
    |""".trimMargin(), runtimeWiring = { wiring ->
      wiring.type("Query") { type ->
        ...
        .dataFetcher("pets") { env ->
          if (env.getArgument<Any?>("isLoyal") == true) {
            listOf(PetService_Dog(name = "Sparky", wearsCollar = true), PetService_Cat(name =
                "Whiskers", wearsBell = false))}
          else {
            null}
        }
      }
