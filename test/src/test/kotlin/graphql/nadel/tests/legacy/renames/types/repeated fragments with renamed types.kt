...
public class `repeated fragments with renamed types` : NadelLegacyIntegrationTest(query = """
|query(${'$'}var1: ID!) {
|  service(id: ${'$'}var1) {
|    __typename
|    dependedOn {
...
|  }
|}
|""".trimMargin(), variables = mapOf("var1" to "service-1"), services = listOf(Service(name="Users",
    overallSchema="""
    |type Query {
      ...
      wiring.type("Query") { type ->
        type.dataFetcher("service") { env ->
          if (env.getArgument<Any?>("id") == "service-1") {
            Users_Service(dependedOn = Users_RelationshipConnection(nodes =
                listOf(Users_Relationship(endService = null, id = "relationship-1", startService =
                Users_Service(dependedOn = null, dependsOn = null, id = "service-1", name =
                "GraphQL Gateway"), type = "unsure"))), dependsOn =
                Users_RelationshipConnection(nodes = listOf()), id = "service-0", name =
                "API Gateway")}
          else {
            null}
        }
  ...
  private data class Users_Relationship(
    ...
    public val startService: Users_Service? = null,
    public val endService: Users_Service? = null,
  ) : Users_Node

  private data class Users_RelationshipConnection(
    public val nodes: List<Users_Relationship?>? = null,
  )

  private data class Users_Service(
    public val id: String? = null,
    public val name: String? = null,
    public val dependedOn: Users_RelationshipConnection? = null,
    public val dependsOn: Users_RelationshipConnection? = null,
  )
}
