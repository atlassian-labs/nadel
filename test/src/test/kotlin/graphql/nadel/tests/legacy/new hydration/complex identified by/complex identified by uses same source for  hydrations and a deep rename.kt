...
public class `complex identified by uses same source for  hydrations and a deep rename` :
    ...
    |""".trimMargin(), runtimeWiring = { wiring ->
      wiring.type("Query") { type ->
        type.dataFetcher("foos") { env ->
          listOf(Foo_Foo(fooId = "Foo-1", issue = Foo_Issue(`field` = "hmm-1")), Foo_Foo(fooId =
              "Foo-2", issue = Foo_Issue(`field` = "hmm-2")), Foo_Foo(fooId = "Foo-3", issue =
        ...
        .dataFetcher("details") { env ->
          ...
          else if (env.getArgument<Any?>("detailIds") == listOf("Foo-4")) {
            listOf(Foo_Detail(detailId = "Foo-4", name = "Four Apples"))}
          else {
            null}
        }
      }
