...
public class `hydrating using json data as arg` : NadelLegacyIntegrationTest(query = """
|query {
      ...
      wiring.type("Query") { type ->
        type.dataFetcher("foo") { env ->
          if (env.getArgument<Any?>("input") == mapOf("something" to true, "answer" to "42")) {
            Service_Foo(baz = mapOf("id" to "102", "appConfig" to mapOf("status" to "active",
                "bounce" to false)))}
          else {
            null}
        }
    ...
    |""".trimMargin(), runtimeWiring = { wiring ->
      wiring.type("Query") { type ->
        type.dataFetcher("baz") { env ->
          if (env.getArgument<Any?>("data") == mapOf("id" to "102", "appConfig" to mapOf("status" to
              "active", "bounce" to false))) {
            Baz_Foo(id = "10000")}
          else {
            null}
        }
