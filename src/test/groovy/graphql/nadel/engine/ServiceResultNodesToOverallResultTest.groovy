...
class ServiceResultNodesToOverallResultTest extends Specification {
    ...
    def "maps ResultNode with a field rename"() {
        ...
        * This simulates a field from a -> x and b -> y
        ...
        1 * fetchedAnalysisMapper.mapFetchedValueAnalysis(originalAnalysis1, overallSchema, rootExecutionStepInfo, false, false, [:], typeRenameMappings) >> mappedAnalysis1
        1 * fetchedAnalysisMapper.mapFetchedValueAnalysis(originalAnalysis2, overallSchema, rootExecutionStepInfo, false, false, [:], typeRenameMappings) >> mappedAnalysis2

        when:
        def newRoot = serviceResultNodesToOverallResult.convert(rootResultNode, overallSchema, rootExecutionStepInfo, [:], typeRenameMappings)
