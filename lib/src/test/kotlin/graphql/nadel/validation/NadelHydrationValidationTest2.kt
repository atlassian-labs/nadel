...
class NadelHydrationValidationTest2 {
    ...
    @Test
    fun `union hydration cannot hydrate from field whose output type is not in union`() {
        val fixture = NadelValidationTestFixture(
            overallSchema = mapOf(
                "activity" to /* language=GraphQL*/ """
                    type Query {
        ...
        assertTrue(errors.singleOrNull() is NadelHydrationUnionMissingTypeError)
        val error = errors.single() as NadelHydrationUnionMissingTypeError
        assertTrue(error.virtualField.name == "data")
        assertTrue(error.missingTypes.map { it.name } == listOf("Page"))
    }
}
