...
class NadelSchemaDefinitionTraverserElementTest {
    ...
    fun `Directive coordinates returns correct NadelDirectiveCoordinates`() {
        ...
        val directiveDefinition = document.definitions[0] as DirectiveDefinition
    ...
    @Test
    fun `AppliedDirective coordinates returns correct NadelAppliedDirectiveCoordinates for ObjectType`() {
        val document = Parser().parseDocument(
            """
        ...
        val objectTypeDefinition = document.definitions[0] as ObjectTypeDefinition
        val objectTypeTraverserElement = NadelSchemaDefinitionTraverserElement.ObjectType(objectTypeDefinition)
        val appliedDirective = objectTypeDefinition.directives[0]
        val appliedDirectiveTraverserElement = NadelSchemaDefinitionTraverserElement.AppliedDirective(objectTypeTraverserElement, appliedDirective)

        // When
        val coordinates = appliedDirectiveTraverserElement.coordinates()

        // Then
        assertTrue(coordinates == NadelObjectCoordinates("User").appliedDirective("auth"))
    }

    @Test
    fun `AppliedDirective coordinates returns correct NadelAppliedDirectiveCoordinates for FieldDefinition`() {
        val document = Parser().parseDocument(
            """
                type User {
        ...
        val objectTypeDefinition = document.definitions[0] as ObjectTypeDefinition
        val objectTypeTraverserElement = NadelSchemaDefinitionTraverserElement.ObjectType(objectTypeDefinition)
        val idFieldDefinition = objectTypeDefinition.fieldDefinitions[0]
        val idFieldTraverserElement = NadelSchemaDefinitionTraverserElement.FieldDefinition(objectTypeTraverserElement, idFieldDefinition)
        val appliedDirective = idFieldDefinition.directives[0]
        val appliedDirectiveTraverserElement = NadelSchemaDefinitionTraverserElement.AppliedDirective(idFieldTraverserElement, appliedDirective)

        // When
        val coordinates = appliedDirectiveTraverserElement.coordinates()

        // Then
        assertTrue(coordinates == NadelObjectCoordinates("User").field("id").appliedDirective("deprecated"))
    }

    @Test
    fun `AppliedDirectiveArgument coordinates returns correct NadelAppliedDirectiveArgumentCoordinates`() {
        val document = Parser().parseDocument(
            """
        ...
        val appliedDirective = objectTypeDefinition.directives[0]
        val appliedDirectiveTraverserElement = NadelSchemaDefinitionTraverserElement.AppliedDirective(objectTypeTraverserElement, appliedDirective)
        val roleArgument = appliedDirective.arguments[0]
        val roleArgumentTraverserElement = NadelSchemaDefinitionTraverserElement.AppliedDirectiveArgument(appliedDirectiveTraverserElement, roleArgument)

        // When
        val coordinates = roleArgumentTraverserElement.coordinates()

        // Then
        assertTrue(coordinates == NadelObjectCoordinates("User").appliedDirective("auth").argument("role"))
    }

    @Test
    fun `multiple applied directive argument coordinates are correct`() {
        val document = Parser().parseDocument(
            """
                type User @auth(role: "admin", scope: "read", level: 5) {
                    id: ID!
                    name: String
                }
            """.trimIndent()
        ...
        val appliedDirective = objectTypeDefinition.directives[0]
        val appliedDirectiveTraverserElement = NadelSchemaDefinitionTraverserElement.AppliedDirective(objectTypeTraverserElement, appliedDirective)
        val appliedDirectiveCoords = NadelObjectCoordinates("User").appliedDirective("auth")

        appliedDirective.arguments.forEach { argument ->
            val argumentTraverserElement = NadelSchemaDefinitionTraverserElement.AppliedDirectiveArgument(appliedDirectiveTraverserElement, argument)

            // When
            val coordinates = argumentTraverserElement.coordinates()

            // Then
            assertTrue(coordinates == appliedDirectiveCoords.argument(argument.name))
        }
    }
}
